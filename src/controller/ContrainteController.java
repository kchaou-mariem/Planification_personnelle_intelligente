...
/**
 * Controller REST pour la gestion des contraintes
 * Endpoints:
 * - GET /api/contraintes/utilisateur/{id} - Liste des contraintes d'un utilisateur
 * - GET /api/contraintes/{id} - Détail d'une contrainte
 * - POST /api/contraintes - Créer une contrainte
 * - PUT /api/contraintes/{id} - Modifier une contrainte
 * - DELETE /api/contraintes/{id} - Supprimer une contrainte
 * - PATCH /api/contraintes/{id}/toggle - Activer/Désactiver une contrainte
 */
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if (pathInfo == null || pathInfo.equals("/")) {
                // GET /api/contraintes - Toutes les contraintes
                ...
                // GET /api/contraintes/utilisateur/{userId}
                String[] parts = pathInfo.split("/");
                if (parts.length >= 3) {
                    int userId = Integer.parseInt(parts[2]);
                    
                    // Check for actives filter
        ...
        try {
            if (pathInfo != null && pathInfo.length() > 1) {
                ...
                if (existing != null) {
                    ...
                    if (json.has("titre")) existing.setTitre(getJsonString(json, "titre"));
                    if (json.has("type")) existing.setType(TypeContrainte.valueOf(getJsonString(json, "type")));
                    if (json.has("dateHeureDeb")) existing.setDateHeureDeb(LocalTime.parse(getJsonString(json, "dateHeureDeb")));
                    if (json.has("dateHeureFin")) existing.setDateHeureFin(LocalTime.parse(getJsonString(json, "dateHeureFin")));
                    if (json.has("repetitif")) existing.setRepetitif(json.get("repetitif").getAsBoolean());
    ...
    protected void doPatch(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if (pathInfo != null && pathInfo.contains("/toggle")) {
                // PATCH /api/contraintes/{id}/toggle
