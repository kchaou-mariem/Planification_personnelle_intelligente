    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if ("/register".equals(pathInfo)) {
                ...
                boolean success = utilisateurService.inscrire(user);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if (pathInfo != null && pathInfo.length() > 1) {
                ...
                response.addProperty("succes", true);
                response.add("utilisateurs", gson.toJsonTree(utilisateurService.getAll()));
            }
        ...
        try {
            if (pathInfo != null && pathInfo.length() > 1) {
                ...
                if (existingUser != null) {
                    ...
                    if (json.has("nom")) existingUser.setNom(getJsonString(json, "nom"));
                    if (json.has("prenom")) existingUser.setPrenom(getJsonString(json, "prenom"));
                    if (json.has("age") && !json.get("age").isJsonNull()) {
                        existingUser.setAge(json.get("age").getAsInt());
                    }
                    if (json.has("genre")) existingUser.setGenre(getJsonString(json, "genre"));
                    if (json.has("poste")) existingUser.setPoste(getJsonString(json, "poste"));
                    
                    boolean success = utilisateurService.modifier(existingUser);
