...
public class ConflitDAOImpl implements ConflitDAO {
    ...
    public boolean supprimer(Long idConflit) {
        // D'abord supprimer les liens dans conflit_activite
        String sqlLiens = "DELETE FROM conflit_activite WHERE id_conflit = ?";
        String sqlConflit = "DELETE FROM conflit WHERE id_conflit = ?";

        try (Connection conn = Connect.getConnection()) {
            // 1. Supprimer les liens
            try (PreparedStatement stmt = conn.prepareStatement(sqlLiens)) {
                stmt.setLong(1, idConflit);
                stmt.executeUpdate();
            }
            
            // 2. Supprimer le conflit
            try (PreparedStatement stmt = conn.prepareStatement(sqlConflit)) {
                stmt.setLong(1, idConflit);
                return stmt.executeUpdate() > 0;
            }
        } catch (SQLException e) {
    ...
    public List<Conflit> getByActivite(Long idActivite) {
        // Note: Cette requête nécessite une table de liaison conflit_activite ou un
        // champ id_activite dans conflit
     ...
     * Convertit les valeurs d'enum de la BD (français avec espaces) vers les
     * constantes Java
     */
    ...
    /**
     * Convertit les valeurs d'enum de la BD (français) vers les constantes Java
     * (anglais)
     */
