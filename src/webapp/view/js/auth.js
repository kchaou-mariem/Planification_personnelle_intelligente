...
const Auth = {
    ...
    try {
        ...
        console.log('📥 Réponse API:', response); // DEBUG IMPORTANT
        
        // ATTENTION : ton backend retourne "succes" (sans accent !)
        if (response.succes && response.utilisateur) {
            console.log('✅ Connexion OK, stockage...');
            this.setUser(response.utilisateur);
            if (response.token) {
                this.setToken(response.token);
            }
            return { success: true, user: response.utilisateur }; // <-- ICI "success" avec double s
        }
